...
#endif
int eXosip_listen_release(struct eXosip_t *excontext)
{
  ...
  //  eXtl = excontext->eXtl;
    ...
    if (excontext->eXtl->proto_family == IPPROTO_UDP)
    {
        close(((struct eXtludp*)(excontext->eXtludp_reserved))->udp_socket);
    }
    else if (excontext->eXtl->proto_family == IPPROTO_TCP)
    {
        close(((struct eXtltcp*)(excontext->eXtltcp_reserved))->tcp_socket);
    }
    excontext->eXtl = NULL;
...
_eXosip_keep_alive (struct eXosip_t *excontext)
{
  ...
  if (now.tv_sec * 60 + now.tv_usec*1000  < mtimer.tv_sec * 60+mtimer.tv_usec * 1000){
      return;
  }
