...
int
eXosip_insubscription_build_answer (struct eXosip_t *excontext, int tid, int status, osip_message_t ** answer,const char *to_tag)
{
  int i = -1;
  eXosip_dialog_t *jd = NULL;
  eXosip_notify_t *jn = NULL;
  ...
  *answer = NULL;
  ...
  if (to_tag)
      jd->d_dialog->local_tag = osip_strdup(to_tag);
...
//     fprintf(stdout,"transaction->state:%d",transaction->state);
//        return OSIP_WRONG_STATE;
//  }
  if (transaction != NULL) {
    if (!(transaction->state == NICT_TERMINATED || transaction->state == NIST_TERMINATED || transaction->state == NICT_COMPLETED || transaction->state == NIST_COMPLETED)){
     fprintf(stderr,"transaction->state:%d",transaction->state);
        return OSIP_WRONG_STATE;
    }
  }
...
eXosip_insubscription_automatic (struct eXosip_t *excontext, eXosip_event_t * evt)
{
  ...
  if (osip_strcasecmp (event_header->hvalue, "dialog") == 0) {
    ...
    if (evt->type == EXOSIP_IN_SUBSCRIPTION_NEW) {
      ...
      i = eXosip_insubscription_build_answer (excontext, evt->tid, 202, &answer,NULL);
      if (i == 0) {
