...
_tcp_tl_send_sockinfo (struct _tcp_stream *sockinfo, const char *msg, int msglen)
{
  ...
  while (1) {
    ...
    {
        OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO3, NULL, "TCP wrote %s\n", msg));
    }
    break;
  }
  return OSIP_SUCCESS;
  ...
  for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
    if (reserved->socket_tab[pos].socket > 0) {
      ...
      if (excontext->keep_alive > 0) {
        ...
        OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO5, NULL, "tcp_tl_keepalive  msg:\n%s\n", (const void *)buf));
