...
udp_tl_read_message (struct eXosip_t *excontext, fd_set * osip_fdset, fd_set * osip_wrset)
{
  ...
  if (FD_ISSET (reserved->udp_socket, osip_fdset)) {
    ...
    OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "----received msg:%s",buf));
    if (i > 5) {
      ...
      if (err != 0) {
        ...
        OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL, "Message received from: %s:%i\n", src6host, recvport));
        OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO5, NULL, "Message received from ip:%s, port:%d, buff:\n%s\n", src6host, recvport, (const void *)buf));
      }
#endif
...
udp_tl_send_message (struct eXosip_t *excontext, osip_transaction_t * tr, osip_message_t * sip, char *host, int port, int out_socket)
{
  ...
  OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO5, NULL, "msg:%d,%s",length,(const void *) message));
...
#endif
    OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO5, NULL, "msg failed:%s",(const void *) message));
  ...
  OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO5, NULL, "UDP wrote remote ip:%s, port:%d, msg:\n%s\n", host, port, (unsigned char*)message));
  ...
  for (jr = excontext->j_reg; jr != NULL; jr = jr->next) {
    if (jr->len > 0) {
      if (sendto (reserved->udp_socket, (const void *) buf, 4, 0, (struct sockaddr *) &(jr->addr), jr->len) > 0) {
        ...
        OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO5, NULL, "eXosip: Keep Alive sent on UDP buff:\n%s\n", buf));
      }
    }
  }
  return OSIP_SUCCESS;
