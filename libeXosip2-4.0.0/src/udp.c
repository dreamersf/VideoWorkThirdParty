...
_eXosip_process_new_subscribe (struct eXosip_t *excontext, osip_transaction_t * transaction, osip_event_t * evt)
{
  ...
  //sufu修改部分代码，个别系统直接来一个续订指令，也需要我们识别为新的订阅，并正确返回结果
  if (to_tag != NULL) {
      OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_ERROR, NULL, "ERROR: Existing To-Tag in new SUBSCRIBE -> should reject with 481,but the system decide to ignore\n"));
  }
#if 0
  if (to_tag != NULL) {
