...
static sf_osip_trace_func_t *sf_trace_func = 0;
...
void
sf_osip_trace_initialize_func (osip_trace_level_t level, sf_osip_trace_func_t * func)
{
  int i = 0;
...
osip_trace (char *filename_long, int li, osip_trace_level_t level, FILE * f, char *chfr, ...)
{
  ...
  if (logfile == NULL && use_syslog == 0 && trace_func == NULL&& sf_trace_func == NULL) {       /* user did not initialize logger.. */
    return 1;
  }
#endif
  ...
  if (f == NULL && trace_func == NULL && sf_trace_func == NULL)
    f = logfile;
...
//  }
  ...
  else if (trace_func || sf_trace_func) {
    char szLog[1500]="";
    va_start (ap, chfr);
    vsprintf (szLog, chfr, ap);
    va_end (ap);
    sf_trace_func (fi, li, level, szLog);
  }
